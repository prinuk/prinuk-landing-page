...
function buildWeightSummaryHtml(data, settings) {
  ...
  items.forEach((it) => {
    if (it.department !== lastDept) {
      rows += '<tr class="dept"><td colspan="4">' + escapeHtml(it.department || 'אחר') + '</td></tr>';
    ...
    // Collected so far, in the row's primary unit (kg for weight-priced, else units).
    const collected = it.weightPriced
      ? (it.collectedWeightKg ? fmtNum(it.collectedWeightKg) + ' ק״ג' : '—')
      : (it.collectedUnits ? fmtNum(it.collectedUnits) + ' יח׳' : '—');
    rows += '<tr><td>' + escapeHtml(it.name) + flag + '</td>' +
      '<td class="num">' + fmtNum(it.totalUnits) + '</td>' +
      '<td class="num">' + kg + '</td>' +
      '<td class="num">' + collected + '</td></tr>';
  });
  ...
  return [
    ...
    '<table><thead><tr><th>מוצר</th><th class="num">יחידות</th><th class="num">ק״ג משוער</th><th class="num">נאסף עד כה</th></tr></thead><tbody>',
    rows,
    '</tbody></table>',
    '</body></html>',
  ].join('');
