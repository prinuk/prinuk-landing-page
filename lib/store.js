    ...
    for (const it of existing) {
      ...
      if (picked) {
        ...
        if (isWeight) {
          ...
          // unit-priced: collected amount × unit price; defaults to the ordered qty.
          const qRaw = inp ? inp.actualQuantity : undefined;
          const q = qRaw != null && qRaw !== '' && isFinite(Number(qRaw)) && Number(qRaw) >= 0 ? Number(qRaw) : Number(it.quantity);
          set.actualQuantity = String(q);
          set.actualWeightKg = null;
          set.actualLineTotalAgorot = it.unitPriceAgorot ? Math.round(q * it.unitPriceAgorot) : it.lineTotalAgorot;
        }
