...
// Bulk status change for the product list (team dashboard multi-select).
async function bulkUpdateProductStatus(ids, state) {
  const allowed = ['active', 'oos', 'hidden'];
  if (allowed.indexOf(state) === -1) return { ok: false, updated: 0 };
  const clean = (Array.isArray(ids) ? ids : []).map(String).filter(isUuid);
  if (!clean.length) return { ok: true, updated: 0 };
  await db.update(productsT).set({ state, updatedAt: new Date() }).where(inArray(productsT.id, clean));
...
const EDITABLE_SETTINGS = [
  ...
  'telegramPickedChatId', 'contactPhone', 'contactEmail',
];
