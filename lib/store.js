...
const { PROVIDER: PAYMENT_PROVIDER, getPaymentAdapter } = require('./payments');
...
async function readOrderForDashboard(orderId) {
  ...
  let payment = { method: o.paymentMethod || 'cash', status: o.paymentStatus || 'none', invoiceUrl: '' };
  if (o.paymentMethod === 'credit') {
    const prow = (await db.select().from(paymentsT).where(eq(paymentsT.orderId, o.id)))[0];
    if (prow) {
      payment = { method: 'credit', status: prow.status || o.paymentStatus || 'none', invoiceUrl: prow.invoiceUrl || '' };
    }
  }
...
async function chargeOrder(orderCode) {
  ...
  if (orderDiscount > 0) items.push({ name: 'הנחה', quantity: 1, lineTotalAgorot: -orderDiscount });
  if (deliveryFee > 0) items.push({ name: 'משלוח', quantity: 1, lineTotalAgorot: deliveryFee });
...
async function reviewAndCharge(orderCode, payload = {}) {
  ...
  for (const r of reqItems) {
    ...
    if (r.id && byId.has(String(r.id))) {
      ...
      const billable = isWeight
        ? (it.actualLineTotalAgorot != null ? it.actualLineTotalAgorot : (it.lineTotalAgorot || 0))
        : (it.unitPriceAgorot ? Math.round(applyUnitDeal(qty, it.unitPriceAgorot, it.dealQty, it.dealPriceAgorot)) : (it.lineTotalAgorot || 0));
      const net = Math.max(0, billable - discount);
      itemsNetTotal += net;
      toUpdate.push({ id: it.id, quantity: String(isWeight ? Number(it.quantity) : qty), actualLineTotalAgorot: net, discountAgorot: discount });
      ...
      const billable = p.priceAgorot ? Math.round(applyUnitDeal(qty, p.priceAgorot, p.dealQty, p.dealPriceAgorot)) : 0;
      const net = Math.max(0, billable - discount);
      itemsNetTotal += net;
      toInsert.push({
        orderId: o.id, productId: p.id, productName: p.name, department: p.department || '',
        mode: 'unit', quantity: String(qty), orderUnit: p.unit || '', unitPriceAgorot: p.priceAgorot || 0,
        priceUnit: p.priceUnit || p.unit || '', dealQty: p.dealQty || null, dealPriceAgorot: p.dealPriceAgorot || null,
        lineTotalAgorot: billable, actualLineTotalAgorot: net, discountAgorot: discount, pickStatus: ITEM_PICK_COLLECTED, sortOrder: 999,
      });
    }
  }
  ...
  const now = new Date();
  await db.transaction(async (tx) => {
    const removeIds = existing.filter((it) => !keepIds.has(String(it.id))).map((it) => it.id);
    if (removeIds.length) await tx.delete(orderItemsT).where(inArray(orderItemsT.id, removeIds));
    for (const u of toUpdate) {
      await tx.update(orderItemsT)
        .set({ quantity: u.quantity, actualLineTotalAgorot: u.actualLineTotalAgorot, discountAgorot: u.discountAgorot })
        .where(eq(orderItemsT.id, u.id));
    }
    if (toInsert.length) await tx.insert(orderItemsT).values(toInsert);
    await tx.update(ordersT)
      .set({ discountAgorot: orderDiscount, actualTotalAgorot: itemsNetTotal, updatedAt: now })
      .where(eq(ordersT.id, o.id));
  });
