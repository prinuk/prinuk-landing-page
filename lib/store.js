...
function mapSummary(o, itemCount, items) {
  return {
    orderId: o.orderCode,
    // Lightweight product breakdown for the dashboard's product search (name +
    // ordered units per line). Only attached by the list query.
...
async function listOrdersForDashboard(scope = {}) {
  ...
  if (ids.length) {
    // One query for the product breakdown; derive both the count and the
    // name+units list per order (used by the dashboard product search).
    const itemRows = await db
      .select({ orderId: orderItemsT.orderId, name: orderItemsT.productName, quantity: orderItemsT.quantity })
      .from(orderItemsT)
      .where(inArray(orderItemsT.orderId, ids));
    for (const r of itemRows) {
      countMap.set(r.orderId, (countMap.get(r.orderId) || 0) + 1);
      if (!itemsMap.has(r.orderId)) itemsMap.set(r.orderId, []);
      itemsMap.get(r.orderId).push({ name: r.name, quantity: Number(r.quantity) || 0 });
    }
  }

  const orders = rows.map((o) => mapSummary(o, countMap.get(o.id) || 0, itemsMap.get(o.id) || []));
...
async function getWeightSummary(scope = {}) {
  ...
  for (const it of items) {
    ...
    // Already collected: only picked (נאסף) lines contribute. Units use the
    // collected count (falling back to the ordered qty); weight-priced use the
    // actual weighed kg, falling back to the estimate for a picked-but-unweighed line.
    if (it.pickStatus === ITEM_PICK_COLLECTED) {
      row.collectedUnits += it.actualQuantity != null ? Number(it.actualQuantity) : Number(it.quantity) || 0;
      if (row.weightPriced) {
        row.collectedWeightKg += it.actualWeightKg != null ? Number(it.actualWeightKg)
          : it.estimatedWeightKg != null ? Number(it.estimatedWeightKg)
          : it.mode === 'kg' ? Number(it.quantity) || 0 : 0;
      }
    }
  }
  ...
  const result = [...map.values()]
    .map((r) => ({
      ...
      collectedUnits: Math.round(r.collectedUnits * 1000) / 1000,
      collectedWeightKg: r.weightPriced ? Math.round(r.collectedWeightKg * 1000) / 1000 : null,
    }))
...
async function readOrderForDashboard(orderId) {
  ...
  return {
    ...
    order: {
      ...
      // Named discount rows for the order-detail UI (amount in ₪, positive).
      discounts: normalizeOrderDiscounts(o).map((d) => ({
        name: d.name, amount: d.amountAgorot / 100, showOnInvoice: d.showOnInvoice,
      })),
    },
  };
}
...
// Normalize the stored discounts jsonb into rows [{ name, amountAgorot,
// showOnInvoice }]. Backward-compat: an order with only the legacy single
// discountAgorot (no rows) becomes one visible "הנחה" row.
function normalizeOrderDiscounts(o) {
  const raw = Array.isArray(o.discounts) ? o.discounts : null;
  if (raw && raw.length) {
    return raw
      .map((d) => ({
        name: String(d.name || '').trim() || 'הנחה',
        amountAgorot: Math.max(0, Math.round(Number(d.amountAgorot) || 0)),
        showOnInvoice: d.showOnInvoice !== false,
      }))
      .filter((d) => d.amountAgorot > 0);
  }
  const legacy = Math.max(0, Math.round(Number(o.discountAgorot) || 0));
  return legacy > 0 ? [{ name: 'הנחה', amountAgorot: legacy, showOnInvoice: true }] : [];
}

// Fold a hidden (not-shown) discount total into the item lines proportionally, so
// the document has no discount line yet the lines still sum to the net amount
// (Cardcom rejects a doc whose lines != the payment). Rounding remainder lands on
// the largest line. Mutates each line's lineTotalAgorot.
function foldHiddenDiscountIntoLines(lines, hiddenAgorot) {
  const gross = lines.reduce((s, l) => s + Math.max(0, l.lineTotalAgorot), 0);
  if (!(hiddenAgorot > 0) || !(gross > 0)) return;
  const take = Math.min(hiddenAgorot, gross);
  let allocated = 0;
  let largest = null;
  lines.forEach((l) => {
    const base = Math.max(0, l.lineTotalAgorot);
    const share = Math.round((base / gross) * take);
    l.lineTotalAgorot = base - share;
    allocated += share;
    if (!largest || base > Math.max(0, largest.lineTotalAgorot)) largest = l;
  });
  // Put any rounding gap on the largest line so the sum is exact.
  const gap = take - allocated;
  if (gap !== 0 && largest) largest.lineTotalAgorot = Math.max(0, largest.lineTotalAgorot - gap);
}

// Parse an incoming discount payload into stored rows. Accepts either a
// `discounts` array ([{ name, amountAgorot, showOnInvoice }]) or the legacy single
// `orderDiscountAgorot`. Returns { rows, totalAgorot }.
function parseDiscountPayload(payload = {}) {
  let rows = [];
  if (Array.isArray(payload.discounts)) {
    rows = payload.discounts
      .map((d) => ({
        name: String(d.name || '').trim() || 'הנחה',
        amountAgorot: Math.max(0, Math.round(Number(d.amountAgorot) || 0)),
        showOnInvoice: d.showOnInvoice !== false,
    ...
    const legacy = Math.max(0, Math.round(Number(payload.orderDiscountAgorot) || 0));
    if (legacy > 0) rows = [{ name: 'הנחה', amountAgorot: legacy, showOnInvoice: true }];
  }
  const totalAgorot = rows.reduce((s, d) => s + d.amountAgorot, 0);
  return { rows, totalAgorot };
}
...
async function buildOrderChargeItems(o) {
  ...
  const discountRows = normalizeOrderDiscounts(o);
  const orderDiscount = discountRows.reduce((s, d) => s + d.amountAgorot, 0);
  ...
  // Hidden discounts: fold into the item lines (no visible line). Shown discounts:
  // add a named negative line each. Either way the lines sum to amountAgorot−delivery.
  const hiddenTotal = discountRows.filter((d) => !d.showOnInvoice).reduce((s, d) => s + d.amountAgorot, 0);
  if (hiddenTotal > 0) foldHiddenDiscountIntoLines(items, hiddenTotal);
  discountRows.filter((d) => d.showOnInvoice).forEach((d) => {
    items.push({ name: d.name || 'הנחה', quantity: 1, lineTotalAgorot: -d.amountAgorot, vatExempt: true });
  });
...
// Record a client-side card failure (tokenize decline / validation block) that
// never reaches a charge, so there's a queryable trail of WHY a customer's card
// failed at checkout (Cardcom's decline reason lives only in the browser). Never
// throws — logging must not break checkout. No card data is ever passed in.
async function logClientCardFailure(detail = {}) {
  try {
    const clip = (v, n) => String(v == null ? '' : v).slice(0, n);
    await db.insert(auditLogT).values({
      actor: 'customer',
      action: 'payment.client_card_failure',
      detail: {
        stage: clip(detail.stage, 40),
        reason: clip(detail.reason, 400),
        provider: clip(detail.provider, 40),
        name: clip(detail.name, 120),
        phone: clip(detail.phone, 40),
        userAgent: clip(detail.userAgent, 240),
        at: new Date().toISOString(),
      },
    });
    return { ok: true };
  } catch (e) {
    console.error('logClientCardFailure failed:', e && e.message);
    return { ok: false };
  }
}

// ---- QR scan tracking (append-only, in audit_log; no new table) ----------
// A QR code points at /api/qr?c=<code>, which logs one row here and redirects to
// the site. `unique` is true the first time a given browser hits a code (cookie),
// so we can report both total scans and roughly how many different people.
async function logQrScan({ code, unique, userAgent, referer } = {}) {
  try {
    const clip = (v, n) => String(v == null ? '' : v).slice(0, n);
    await db.insert(auditLogT).values({
      actor: 'visitor',
      action: 'qr.scan',
      entity: clip(code, 40) || 'meida',
      detail: {
        unique: !!unique,
        userAgent: clip(userAgent, 240),
        referer: clip(referer, 240),
        at: new Date().toISOString(),
      },
    });
    return { ok: true };
  } catch (e) {
    console.error('logQrScan failed:', e && e.message);
    return { ok: false };
  }
}

// Per-code scan stats for the dashboard: total scans, unique people (first-time
// cookie hits), and the most recent scan. Newest-active code first.
// range: optional { from, to } (ISO or YYYY-MM-DD) to count scans only in that
// window; a plain YYYY-MM-DD `to` covers the whole day.
async function getQrStats(range = {}) {
  const conds = [eq(auditLogT.action, 'qr.scan')];
  if (range && range.from) conds.push(gte(auditLogT.createdAt, new Date(range.from)));
  if (range && range.to) {
    const to = /^\d{4}-\d{2}-\d{2}$/.test(range.to) ? new Date(range.to + 'T23:59:59.999') : new Date(range.to);
    conds.push(lte(auditLogT.createdAt, to));
  }

  const rows = await db
    .select({
      code: auditLogT.entity,
      total: count(),
      uniques: sum(sql`CASE WHEN ${auditLogT.detail}->>'unique' = 'true' THEN 1 ELSE 0 END`),
      last: max(auditLogT.createdAt),
    })
    .from(auditLogT)
    .where(and(...conds))
    .groupBy(auditLogT.entity);
...
// Permanently delete all scans for one QR code (a hard "reset to zero"). Returns
// how many scan rows were removed. Only touches qr.scan audit rows.
async function resetQrCode(code) {
  const c = String(code || '').trim();
  if (!c) return { ok: false, deleted: 0 };
  const deleted = await db
    .delete(auditLogT)
    .where(and(eq(auditLogT.action, 'qr.scan'), eq(auditLogT.entity, c)))
    .returning({ id: auditLogT.id });
...
// Collect payment for one or more orders as a SINGLE invoice. Only orders WITHOUT
// a saved card belong here (carded orders are charged on their own card). `tenders`
// is a list of { method, amountAgorot, reference } that must sum to the combined
// total; a `credit` tender is charged on a card tokenized in the browser
// (newCardLpId). Each order is then marked paid and linked to the one invoice —
// the orders themselves are NOT merged.
async function collectPayment(payload = {}) {
  ...
  if (creditTender) {
    if (payload.newCardLpId) {
      // New card just tokenized in the browser (CreateTokenOnly) → resolve its token
      // and charge the card portion server-side (no separate document).
      const first = orders[0];
      const saved = await adapter.saveCard({ lowProfileId: String(payload.newCardLpId), customer: { fullName: first.fullName, phone: first.phone, email: first.email } });
      if (!saved.ok || !saved.customerRef) return { ok: false, reason: 'card-save-failed', error: saved.error || 'כרטיס האשראי לא אומת.' };
      const charged = await adapter.chargeToken({
        customerRef: saved.customerRef, cardExpiration: saved.cardExpiry || '',
...
async function applyReviewAdjustments(o, payload = {}) {
  ...
  const { rows: discountRows, totalAgorot: orderDiscount } = parseDiscountPayload(payload);
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(ordersT)
      .set({ discountAgorot: orderDiscount, discounts: discountRows, deliveryFeeAgorot: deliveryFee, actualTotalAgorot: itemsNetTotal, updatedAt: now })
      .where(eq(ordersT.id, o.id));
  });
...
// Persist the order's discount rows WITHOUT charging or issuing anything ("save
// the discount"). Updates the stored breakdown + total and reflects it in the
// displayed estimate; final billing recomputes at charge/collection time.
async function saveOrderDiscounts(orderCode, payload = {}) {
  const o = await getOrderByCode(orderCode);
  if (!o) return { ok: false, reason: 'notfound' };
  const { rows, totalAgorot } = parseDiscountPayload(payload);
  const deliveryFee = o.deliveryFeeAgorot || 0;
  // Reflect the discount in the shown estimate (grandTotal = items − discount + delivery).
  const estItems = o.estimatedTotalAgorot || 0;
  const grandTotalAgorot = Math.max(0, estItems - totalAgorot) + deliveryFee;
  await db.update(ordersT)
    .set({ discountAgorot: totalAgorot, discounts: rows, grandTotalAgorot, updatedAt: new Date() })
    .where(eq(ordersT.id, o.id));
  return { ok: true, discountAgorot: totalAgorot, discounts: rows };
}
...
async function adminUpdateOrder(orderCode, payload = {}) {
  ...
  for (const r of reqItems) {
    ...
    // Add a weight-priced item BY ACTUAL WEIGHT (kg): price = weight × price/kg.
    if (isKg && r.weightKg != null && r.weightKg !== '') {
      const w = Math.max(0, Number(r.weightKg) || 0);
      if (!(w > 0)) continue;
      const lineTotal = Math.round(w * (p.priceAgorot || 0));
      estimatedTotalAgorot += lineTotal;
      toInsert.push({
        orderId: o.id, productId: p.id, productName: p.name, department: p.department || '',
        mode: 'kg', quantity: '1', orderUnit: p.unit || '',
        ...
        vatExempt: p.vatExempt !== false,
        lineTotalAgorot: lineTotal, estimatedWeightKg: String(w),
        sortOrder: 999,
      });
      continue;
    }
...
// Quick-add a SINGLE product to an existing order (team) — the one-tap "הוסף מוצר"
// on the order screen. Adds by units (quantity) or by actual weight (kg) for
// weight-priced products, WITHOUT touching the existing lines (so it always
// creates a new row, even for a product already in the order), then recomputes
// the order totals.
async function addOrderItem(orderCode, payload = {}) {
  const o = await getOrderByCode(orderCode);
  if (!o) return { ok: false, reason: 'notfound' };
  const pid = String(payload.productId || '');
  if (!isUuid(pid)) return { ok: false, reason: 'no-product' };
  const p = (await db.select().from(productsT).where(eq(productsT.id, pid)))[0];
  if (!p) return { ok: false, reason: 'no-product' };

  const priceIsKg = getUnitType(p.priceUnit || p.unit) === 'kg';
  const P = p.priceAgorot || 0; // ₪ per the product's priceUnit
  // kg per unit — admin override, else the name-based estimate. Lets us price the
  // NON-native measure (a kg product by units, or a unit product by kg).
  const W = p.weightPerUnitKg != null ? Number(p.weightPerUnitKg) : (getEstimatedUnitWeightKg(p.name) || null);
  // Chosen measure: explicit ('kg'|'unit'), else infer from which amount was sent,
  // else the product's natural unit.
  const measure = payload.unit === 'kg' ? 'kg'
    : payload.unit === 'unit' ? 'unit'
    : (payload.weightKg != null && payload.weightKg !== '') ? 'kg'
    : (payload.quantity != null && payload.quantity !== '') ? 'unit'
    : (priceIsKg ? 'kg' : 'unit');
  const trim3 = (n) => Math.round(Number(n) * 1000) / 1000;

  let row;
  if (measure === 'kg') {
    const w = Math.max(0, Number(payload.weightKg != null ? payload.weightKg : payload.quantity) || 0);
    if (!(w > 0)) return { ok: false, reason: 'bad-weight' };
    // ₪/kg: native for kg products; for a unit product convert P(₪/unit)÷W(kg/unit).
    const pricePerKg = priceIsKg ? P : (W ? Math.round(P / W) : null);
    row = {
      orderId: o.id, productId: p.id, productName: p.name, department: p.department || '',
      mode: 'kg', quantity: '1', orderUnit: 'ק"ג',
      unitPriceAgorot: pricePerKg != null ? pricePerKg : P, priceUnit: 'ק"ג',
      dealQty: null, dealPriceAgorot: null, vatExempt: p.vatExempt !== false,
      lineTotalAgorot: pricePerKg != null ? Math.round(w * pricePerKg) : null,
      estimatedWeightKg: String(w),
      note: String(payload.note || '').trim(), sortOrder: 999,
    };
  } else {
    const qty = Math.max(1, Math.round(Number(payload.quantity != null ? payload.quantity : payload.weightKg) || 1));
    let lineTotal;
    let unitPrice;
    if (priceIsKg) {
      // kg-priced product entered BY UNITS → ₪/unit = P(₪/kg) × W(kg/unit).
      const pricePerUnit = W ? Math.round(P * W) : null;
      unitPrice = pricePerUnit != null ? pricePerUnit : P;
      lineTotal = pricePerUnit != null ? qty * pricePerUnit : null; // null → priced/counted at collection
    } else {
      unitPrice = P;
      lineTotal = P ? Math.round(applyUnitDeal(qty, P, p.dealQty, p.dealPriceAgorot)) : null;
    }
    row = {
      orderId: o.id, productId: p.id, productName: p.name, department: p.department || '',
      mode: 'unit', quantity: String(qty), orderUnit: 'יחידות',
      unitPriceAgorot: unitPrice, priceUnit: 'יחידות',
      dealQty: priceIsKg ? null : (p.dealQty || null), dealPriceAgorot: priceIsKg ? null : (p.dealPriceAgorot || null),
      vatExempt: p.vatExempt !== false,
      lineTotalAgorot: lineTotal,
      estimatedWeightKg: (priceIsKg && W) ? String(trim3(qty * W)) : null,
      note: String(payload.note || '').trim(), sortOrder: 999,
    };
  }
  ...
  const now = new Date();
  await db.transaction(async (tx) => {
    await tx.insert(orderItemsT).values(row);
    // Recompute order totals from all items (mirrors buildOrderChargeItems' basis).
    const items = await tx.select().from(orderItemsT).where(eq(orderItemsT.orderId, o.id));
    let estimated = 0;
    let unpriced = 0;
    let actual = 0;
    for (const it of items) {
      if (it.lineTotalAgorot != null) estimated += it.lineTotalAgorot; else unpriced += 1;
      if (it.pickStatus !== ITEM_PICK_MISSING) {
        actual += (it.actualLineTotalAgorot != null ? it.actualLineTotalAgorot : (it.lineTotalAgorot || 0));
      }
    }
    const discount = o.discountAgorot || 0;
    const delivery = o.deliveryFeeAgorot || 0;
    const set = {
      estimatedTotalAgorot: estimated,
      unpricedItemCount: unpriced,
      grandTotalAgorot: Math.max(0, estimated - discount) + delivery,
      updatedAt: now,
    };
    // Keep the collected (final) total in sync only if the order was already weighed.
    if (o.actualTotalAgorot != null) set.actualTotalAgorot = actual;
    await tx.update(ordersT).set(set).where(eq(ordersT.id, o.id));
  });
...
async function updateProduct(id, product) {
  ...
  if (product.details !== undefined) set.details = String(product.details || '').trim();
...
const EDITABLE_SETTINGS = [
  ...
  'announcement', 'announcementProductId',
