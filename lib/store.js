...
// Issue a חשבונית מס (tax invoice, no receipt line) for an order paid OUTSIDE
// Cardcom — bank/wire transfer, Bit, Paybox, cash, etc. Bills the same collected
// amount as a card charge; `note` records how it was paid (method + reference)
// on the document. Deliberately NOT recorded as paid in Cardcom (no cash/cheque
// line) so external payments don't mix into the cash report.
async function issueOrderDocument(orderCode, opts = {}) {
  const o = await getOrderByCode(orderCode);
  if (!o) return { ok: false, reason: 'notfound' };
  const adapter = getPaymentAdapter();
  if (typeof adapter.createDocument !== 'function') return { ok: false, reason: 'unsupported' };
  ...
  const res = await adapter.createDocument({
    ...
    externalId: o.orderCode,
    docType: 'TaxInvoice',
    paymentNote: String(opts.note || '').trim(),
...
async function reviewAndIssueDocument(orderCode, payload = {}) {
  ...
  const applied = await applyReviewAdjustments(o, payload);
  if (!applied.ok) return applied;
  return issueOrderDocument(orderCode, { note: payload.note });
}
...
async function createManualOrder(payload = {}) {
  ...
  if (payload.issueDocument) {
    const doc = await issueOrderDocument(orderCode, { note: payload.docNote });
