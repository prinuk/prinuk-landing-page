...
async function updateOrderCollection(orderId, { member, items, closeMissing } = {}) {
  ...
  await db.transaction(async (tx) => {
    for (const it of existing) {
      ...
      if (picked) {
        // For kg-priced items, use the collected weight × price/kg; otherwise the
        // stored (fixed/estimated) line total.
        const isWeight = getUnitType(it.priceUnit) === 'kg';
        const wRaw = inp ? inp.actualWeightKg : undefined;
        const hasWeight = isWeight && wRaw != null && wRaw !== '' && isFinite(Number(wRaw)) && Number(wRaw) > 0;
        const actualLine = hasWeight ? Math.round(Number(wRaw) * (it.unitPriceAgorot || 0)) : it.lineTotalAgorot;
        set.actualWeightKg = hasWeight ? String(Number(wRaw)) : null;
        set.actualLineTotalAgorot = actualLine;
        if (actualLine != null) actualTotalAgorot += actualLine;
      } else {
