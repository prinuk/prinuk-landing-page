...
async function addOrderItem(orderCode, payload = {}) {
  ...
  let row;
  if (isKg && payload.weightKg != null && payload.weightKg !== '') {
    const w = Math.max(0, Number(payload.weightKg) || 0);
    ...
    row = {
      ...
      dealQty: p.dealQty || null, dealPriceAgorot: p.dealPriceAgorot || null,
      vatExempt: p.vatExempt !== false,
      lineTotalAgorot: Math.round(w * (p.priceAgorot || 0)), estimatedWeightKg: String(w),
      note: String(payload.note || '').trim(), sortOrder: 999,
    };
  } else {
    const qty = Math.max(1, Math.round(Number(payload.quantity) || 1));
    const hasWpu = isKg && p.weightPerUnitKg != null;
    const snap = {
      priceUnit: p.priceUnit || p.unit, unitPriceAgorot: p.priceAgorot || 0, lineTotalAgorot: 0,
      mode: hasWpu ? 'unit' : (isKg ? 'kg' : 'unit'),
      estimatedWeightPerUnitKg: hasWpu ? Number(p.weightPerUnitKg) : null,
    };
    const line = recomputeLine(snap, qty);
    row = {
      ...
      dealQty: p.dealQty || null, dealPriceAgorot: p.dealPriceAgorot || null,
      vatExempt: p.vatExempt !== false,
      lineTotalAgorot: line.lineTotalAgorot, estimatedWeightKg: line.estimatedWeightKg,
      estimatedWeightPerUnitKg: snap.estimatedWeightPerUnitKg != null ? String(snap.estimatedWeightPerUnitKg) : null,
      note: String(payload.note || '').trim(), sortOrder: 999,
    };
  }
