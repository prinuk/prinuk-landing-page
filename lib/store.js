...
// Per-sale order timeline — for the comparison chart: cumulative orders vs hours
// ELAPSED since each sale started (its first order). One series per sale, aligned
// at t=0, so sales' ordering pace can be compared regardless of calendar date.
async function getOrdersTimeline() {
  ...
  for (const r of rows) {
    ...
    bySale.get(r.saleName).push(new Date(r.createdAt).getTime());
  }
  return [...bySale.entries()]
    .map(([saleName, times]) => {
      times.sort((a, b) => a - b);
      const start = times[0];
      const offsetsHours = times.map((t) => Math.round(((t - start) / 3600000) * 100) / 100);
      return { saleName, total: times.length, last: times[times.length - 1], offsetsHours };
    })
    .sort((a, b) => b.last - a.last)
    .map(({ saleName, total, offsetsHours }) => ({ saleName, total, offsetsHours }));
}
