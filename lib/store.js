 ...
 */
const { eq, inArray, count } = require('drizzle-orm');
...
async function writeOrder(order) {
  ...
  await db.transaction(async (tx) => {
    ...
    const [o] = await tx
      .insert(ordersT)
      .values({ ...orderRowValues(order, now), customerId, status: ORDER_STATUS_NEW, createdAt: now })
...
async function listOrdersForDashboard() {
  ...
  const rows = await db.select().from(ordersT);
  const counts = await db
    .select({ orderId: orderItemsT.orderId, n: count() })
    .from(orderItemsT)
    .groupBy(orderItemsT.orderId);
  const countMap = new Map(counts.map((c) => [c.orderId, Number(c.n)]));
