  ...
  if (token) {
    ...
    if (payment) {
      await db.update(paymentsT)
        .set({ provider: PAYMENT_PROVIDER, method: 'credit', status: 'authorized', providerCustomerRef: saved.customerRef || null, updatedAt: t })
        .where(eq(paymentsT.id, payment.id));
      payment.providerCustomerRef = saved.customerRef;
      payment.status = 'authorized';
    } else {
      const [row] = await db.insert(paymentsT)
        .values({ orderId: o.id, provider: PAYMENT_PROVIDER, method: 'credit', status: 'authorized', providerCustomerRef: saved.customerRef || null, createdAt: t, updatedAt: t })
        .returning({ id: paymentsT.id });
      payment = { id: row.id, providerCustomerRef: saved.customerRef, status: 'authorized' };
    }
...
async function createManualOrder(payload = {}) {
  ...
  if (wantsCredit) {
    ...
    await db.insert(paymentsT).values({
      ...
      providerCustomerRef: saved.customerRef || null, createdAt: now, updatedAt: now,
    });
