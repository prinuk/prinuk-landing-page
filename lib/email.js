...
function appendItemText(lines, items) {
  (items || []).forEach(line => {
    ...
    lines.push('- ' + line.product.name + ': ' + formatQuantity(line.quantity) + ' ' + line.orderUnit + ' | ' + total);
...
function buildOrderEmailHtml(settings, title, intro, order, items) {
  ...
  const rows = (items || []).map(line => {
    ...
    const noteHtml = line.note
      ? '<div style="font-size:12px;color:#667074;margin-top:3px;font-weight:normal;">הערה: ' + escapeHtml(line.note) + '</div>'
      : '';

    return [
      '<tr>',
      '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;font-weight:bold;">', escapeHtml(line.product.name), noteHtml, '</td>',
      '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;">', escapeHtml(formatQuantity(line.quantity)), '</td>',
      '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;">', escapeHtml(line.orderUnit), '</td>',
