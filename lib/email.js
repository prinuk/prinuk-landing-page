...
function buildBusinessHtml(settings, order, items) {
  return buildOrderEmailHtml(
    ...
    order,
    items,
    order.isUpdate ? { changesHtml: buildChangesEmailHtml(order) } : {}
  );
}

function buildChangesEmailHtml(order) {
  ...
  return [
    ...
    '<div style="font-weight:800;margin-bottom:6px;">🔄 מה השתנה בהזמנה</div>',
    inner,
...
function buildCustomerHtml(settings, order, items) {
  return buildOrderEmailHtml(
    ...
    order,
    items,
    { includeFreshnessNotice: true, editUrl: buildEditOrderUrl(order) }
  );
}

function buildEditOrderButtonHtml(editUrl) {
  return [
    '<div style="text-align:center;margin-bottom:16px;">',
    '<div style="font-size:14px;color:#445;margin-bottom:8px;">צריך לשנות משהו? אפשר לעדכן את ההזמנה עד סגירת ההזמנות.</div>',
...
function buildOrderEmailHtml(settings, title, intro, order, items, options = {}) {
  ...
  return [
    ...
    options.editUrl ? buildEditOrderButtonHtml(options.editUrl) : '',
