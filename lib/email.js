...
async function sendOrderEmail(options, dependencies) {
  try {
    ...
    return {
      status: SENT_STATUS,
      error: mainAttachment ? '' : formatMissingPdfError(options.pdfError),
      success: true,
      skippedEmail: false,
    };
  } catch (error) {
...
// Email clients (notably Gmail) strip data-URI images, so the email logo must
// be a hosted URL. Fall back to the site's own logo asset when none is set.
function getEmailLogoUrl(settings) {
  const configured = String(settings && settings.logoUrl || '').trim();
  return configured || (getOrderSiteBaseUrl() + '/assets/prinuk-logo-round.png');
}

function buildOrderEmailHtml(settings, title, intro, order, items, options = {}) {
  const contactText = buildContactText(settings);
  const logoUrl = getEmailLogoUrl(settings);
  ...
  return [
    ...
    '</td>',
    // Spacer matching the logo cell so the centered title is centered across
    // the full header width, not just the space beside the logo.
    '<td style="width:92px;padding:0 0 12px;" aria-hidden="true"></td>',
    '</tr>',
    '</table>',
...
// ---- Final/completion email (after the order is charged) ----
// Sends the customer the collected (picked) summary + the tax invoice (linked and
// attached). `order` + `items` use the dashboard shape (readOrderForDashboard).
function fmtMoney(n) { return '₪' + (Math.round((Number(n) || 0) * 100) / 100).toFixed(2); }

function buildFinalItemCells(it) {
  const missing = it.pickStatus === 'חסר';
  let qty; let amount;
  if (missing) { qty = 'חסר'; amount = '—'; } else if (it.isWeightPriced) {
    qty = (it.actualWeightKg != null ? it.actualWeightKg : '—') + ' ק״ג';
    amount = fmtMoney(it.actualLineTotal != null ? it.actualLineTotal : it.lineTotal);
  } else {
    qty = (it.actualQuantity != null ? it.actualQuantity : it.quantity) + ' יח׳';
    amount = fmtMoney(it.actualLineTotal != null ? it.actualLineTotal : it.lineTotal);
  }
  return { qty, amount, missing };
}

function buildFinalCustomerHtml(settings, order, items, amount, invoiceUrl) {
  const logoUrl = getEmailLogoUrl(settings);
  const contactText = buildContactText(settings);
  const logoHtml = logoUrl
    ? '<img src="' + escapeHtml(logoUrl) + '" alt="פרינוּק" width="76" height="76" style="display:block;width:76px;height:76px;object-fit:contain;border:0;">'
  ...
  const rows = (items || []).map((it) => {
    const c = buildFinalItemCells(it);
    const style = c.missing ? 'color:#b33636;' : '';
    return '<tr style="' + style + '">'
      + '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;font-weight:bold;">' + escapeHtml(it.name) + '</td>'
      + '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;">' + escapeHtml(c.qty) + '</td>'
      + '<td style="border:1px solid #d9ded6;padding:8px;text-align:right;">' + escapeHtml(c.amount) + '</td>'
      + '</tr>';
  }).join('');
  const invoiceBtn = invoiceUrl
    ? '<div style="margin:16px 0;text-align:center;"><a href="' + escapeHtml(invoiceUrl) + '" style="display:inline-block;background:#1f7a5a;color:#ffffff;text-decoration:none;font-weight:800;padding:12px 22px;border-radius:8px;">צפייה / הורדת הקבלה</a></div>'
    : '';
  return [
    '<div dir="rtl" style="font-family:Arial,Helvetica,sans-serif;color:#1e2528;line-height:1.45;background:#ffffff;">',
    '<div style="max-width:720px;margin:0 auto;padding:18px;">',
    ...
    '<td style="width:92px;padding:0 0 12px;text-align:center;vertical-align:middle;">', logoHtml, '</td>',
    '<td style="padding:0 12px 12px;text-align:center;vertical-align:middle;">',
    '<div style="font-size:26px;font-weight:800;">ההזמנה הושלמה</div>',
    '<div style="font-size:15px;font-weight:700;color:#165a43;margin-top:4px;">פרינוּק - המכירה השבועית</div>',
    contactText ? '<div style="font-size:13px;font-weight:700;color:#165a43;margin-top:4px;">' + escapeHtml(contactText) + '</div>' : '',
    '</td>',
    '<td style="width:92px;padding:0 0 12px;" aria-hidden="true"></td>',
    '</tr>',
    '</table>',
    '<p style="font-size:16px;font-weight:700;margin:0 0 14px;">תודה! ההזמנה שלך נאספה וחויבה. מצורפת קבלה, ולהלן סיכום הליקוט.</p>',
    '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;background:#f7f6f1;border:1px solid #d9ded6;border-radius:8px;margin-bottom:14px;">',
    '<tr><td style="padding:8px;font-weight:bold;color:#165a43;">מספר הזמנה:</td><td style="padding:8px;">', escapeHtml(order.orderId), '</td></tr>',
    '<tr><td style="padding:8px;font-weight:bold;color:#165a43;">לקוח:</td><td style="padding:8px;">', escapeHtml(order.fullName || ''), '</td></tr>',
    '<tr><td style="padding:8px;font-weight:bold;color:#165a43;">שיטת הזמנה:</td><td style="padding:8px;">', escapeHtml(order.fulfillment || ''), '</td></tr>',
    '<tr><td style="padding:8px;font-weight:bold;color:#165a43;">כתובת/איסוף:</td><td style="padding:8px;">', escapeHtml(buildAddressText({ ...order, settings })), '</td></tr>',
    amount != null ? '<tr><td style="padding:8px;font-weight:bold;color:#165a43;">סכום ששולם:</td><td style="padding:8px;font-weight:800;color:#165a43;">' + escapeHtml(fmtMoney(amount)) + '</td></tr>' : '',
    '</table>',
    invoiceBtn,
    '<h3 style="margin:18px 0 8px;color:#165a43;">סיכום הליקוט</h3>',
    '<table width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">',
    '<thead><tr>',
    '<th style="border:1px solid #1f7a5a;background:#1f7a5a;color:#ffffff;padding:8px;text-align:right;">מוצר</th>',
    '<th style="border:1px solid #1f7a5a;background:#1f7a5a;color:#ffffff;padding:8px;text-align:right;">כמות שנאספה</th>',
    '<th style="border:1px solid #1f7a5a;background:#1f7a5a;color:#ffffff;padding:8px;text-align:right;">סכום</th>',
    '</tr></thead><tbody>', rows, '</tbody></table>',
    invoiceUrl ? '<p style="margin:14px 0 0;color:#667074;">📎 מצורפת קבלה (PDF). ניתן גם לצפות בה בקישור למעלה.</p>' : '',
    '</div></div>',
  ].join('');
}

// A4 PDF version of the picked summary for the final email — final amounts only
// (no estimate), with a delivery line when a fee applies (i.e. below the free-
// delivery threshold) and an order discount line when present.
function buildFinalSummaryPdfHtml(settings, order, items, amount) {
  const logoUrl = getEmailLogoUrl(settings);
  const contactText = buildContactText(settings);
  const logoHtml = logoUrl
    ? '<img class="logo" src="' + escapeHtml(logoUrl) + '" alt="פרינוּק" onerror="this.style.display=\'none\'">'
    : '';
  const rows = (items || []).map((it) => {
    const c = buildFinalItemCells(it);
    const style = c.missing ? ' style="color:#b33636;"' : '';
    return '<tr' + style + '><td class="nm">' + escapeHtml(it.name) + '</td><td>' + escapeHtml(c.qty) + '</td><td>' + escapeHtml(c.amount) + '</td></tr>';
  }).join('');
  const deliveryFee = Number(order.deliveryFee) || 0;
  const discount = Number(order.discount) || 0;
  const extra = [
    discount > 0 ? '<tr class="sum"><td colspan="2">הנחה</td><td>-' + escapeHtml(fmtMoney(discount)) + '</td></tr>' : '',
    deliveryFee > 0 ? '<tr class="sum"><td colspan="2">משלוח</td><td>' + escapeHtml(fmtMoney(deliveryFee)) + '</td></tr>' : '',
    amount != null ? '<tr class="total"><td colspan="2">סכום ששולם</td><td>' + escapeHtml(fmtMoney(amount)) + '</td></tr>' : '',
  ].join('');
  return [
    ...
    '.head{display:flex;align-items:center;gap:16px;border-bottom:3px solid #1f7a5a;padding-bottom:14px;margin-bottom:16px;}',
    '.logo{width:64px;height:64px;object-fit:contain;}',
    '.head h1{font-size:22px;margin:0;}.head .meta{font-size:13px;color:#165a43;margin-top:4px;font-weight:700;}',
    '.box{background:#f7f6f1;border:1px solid #d9ded6;border-radius:8px;padding:10px 12px;margin-bottom:14px;}',
    '.box div{margin:2px 0;}.label{font-weight:bold;color:#165a43;}',
    'table{width:100%;border-collapse:collapse;margin-top:6px;}',
    'th,td{border:1px solid #d9ded6;padding:8px;text-align:right;}th{background:#1f7a5a;color:#fff;}',
    'td.nm{font-weight:bold;}tr:nth-child(even) td{background:#fbfcfa;}',
    'tr.sum td{font-weight:700;background:#fff;}tr.total td{font-weight:800;color:#165a43;font-size:16px;background:#eef5f1;}',
    '</style></head><body>',
    '<div class="head">', logoHtml, '<div><h1>פרינוּק — סיכום הזמנה</h1>',
    '<div class="meta">', escapeHtml(order.orderId || ''), contactText ? ' | ' + escapeHtml(contactText) : '', '</div></div></div>',
    '<div class="box">',
    '<div><span class="label">לקוח:</span> ', escapeHtml(order.fullName || ''), '</div>',
    order.phone ? '<div><span class="label">טלפון:</span> ' + escapeHtml(order.phone) + '</div>' : '',
    ...
    '</div>',
    '<table><thead><tr><th>מוצר</th><th>כמות שנאספה</th><th>סכום</th></tr></thead>',
    '<tbody>', rows, extra, '</tbody></table>',
    '</body></html>',
  ].join('');
}

async function createFinalSummaryPdf(settings, order, items, amount) {
  const { renderHtmlToPdf } = require('./pdf-render');
  return renderHtmlToPdf(buildFinalSummaryPdfHtml(settings, order, items, amount));
}

function buildFinalCustomerText(order, items, amount, invoiceUrl) {
  const lines = ['ההזמנה הושלמה - פרינוּק', 'מספר הזמנה: ' + order.orderId, ''];
  lines.push('סיכום הליקוט:');
  (items || []).forEach((it) => {
    const c = buildFinalItemCells(it);
    lines.push('- ' + it.name + ' | ' + c.qty + ' | ' + c.amount);
  });
  if (amount != null) lines.push('', 'סכום ששולם: ' + fmtMoney(amount));
  if (invoiceUrl) lines.push('', 'מצורפת קבלה. צפייה: ' + invoiceUrl);
  return lines.join('\n');
}

async function sendCustomerFinalEmail(settings, order, items, opts = {}) {
  try {
    if (!order || !order.email) return { status: 'לא נמסר מייל', error: '' };
    const invoiceUrl = opts.invoiceUrl || (order.payment && order.payment.invoiceUrl) || '';
    const amount = opts.amount != null ? opts.amount : (order.actualTotal != null ? order.actualTotal : null);

    const extraAttachments = [];
    // Order-summary PDF with the FINAL amounts (+ delivery when a fee applies).
    try {
      const summaryPdf = await createFinalSummaryPdf(settings, order, items, amount);
      if (summaryPdf && summaryPdf.length) extraAttachments.push({ filename: 'סיכום-הזמנה-' + order.orderId + '.pdf', buffer: summaryPdf });
    } catch (e) { /* best-effort: fall back to the HTML summary in the body */ }
    // Best-effort: fetch + attach the invoice PDF (also linked in the body).
    if (invoiceUrl) {
      try {
        const res = await fetch(invoiceUrl);
        if (res.ok) {
          const buf = Buffer.from(await res.arrayBuffer());
          if (buf && buf.length) extraAttachments.push({ filename: 'קבלה-' + order.orderId + '.pdf', buffer: buf });
        }
    ...
    return await sendOrderEmail({
      to: [order.email],
      subject: 'ההזמנה שלך בפרינוּק הושלמה - ' + order.orderId,
      text: buildFinalCustomerText(order, items, amount, invoiceUrl),
      html: buildFinalCustomerHtml(settings, order, items, amount, invoiceUrl),
      replyTo: getCustomerReplyToEmail(),
      order,
      extraAttachments,
    });
  } catch (error) {
    return { status: FAILED_STATUS, error: error.message || 'שליחת מייל סיכום נכשלה.' };
  }
}
