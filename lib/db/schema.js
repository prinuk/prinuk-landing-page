...
const products = pgTable(
  'products',
  {
    ...
    // Optional "X for Y" quantity deal (unit-priced items): dealQty units cost
    // dealPriceAgorot (e.g. 3 for ₪10), applied per full group, remainder at price.
    dealQty: integer('deal_qty'),
    dealPriceAgorot: integer('deal_price_agorot'),
    state: productStateEnum('state').notNull().default('active'),
    // Time-limited availability: orderable only until the daily cutoff time
    // (settings.orderCutoffEnforceTime); shown but disabled afterwards.
    orderCutoff: boolean('order_cutoff').notNull().default(false),
    // Optional sub-group within a category (e.g. יינות / ליקרים under
    // "יינות ואלכוהול"); products are shown grouped under sub-headers, same tab.
    subcategory: text('subcategory').notNull().default(''),
    // Optional display-only volume label for drinks, in millilitres (e.g. 500).
    volumeMl: integer('volume_ml'),
    // Optional short display-only detail line shown under the name on the
    // product card (e.g. "כשר בהשגחת הרבנות"). Managed from the team backoffice.
    details: text('details').notNull().default(''),
    // VAT (מע״מ): fresh produce is exempt; wine/alcohol/etc. are taxable. Web
    // prices are VAT-inclusive — the invoice (Sumit) breaks out VAT on taxable lines.
    vatExempt: boolean('vat_exempt').notNull().default(true),
...
const orders = pgTable(
  'orders',
  {
    ...
    // How the order was created: 'web' = customer via the site, 'manual' = a
    // team-created invoice (חשבונית חדשה / POS). Lets the dashboard separate them.
    ...
    // Order-level discount applied at charge-review (resolved to agorot). This is
    // the TOTAL of all discount rows below (kept for the revenue reports that read
    // it directly); the per-row breakdown lives in `discounts`.
    discountAgorot: integer('discount_agorot'),
    // Named discount rows: [{ name, amountAgorot, showOnInvoice }]. amountAgorot is
    // positive (the amount taken off). showOnInvoice=true → a visible "הנחה" line on
    // the document; false → folded into the item lines (reduces the total, no line).
    ...
    // Manual team override of the displayed payment status, independent of the
    // real processor state: 'paid' | 'unpaid' | 'na' (לא רלוונטי). null = derive
    // from paymentStatus (captured → paid, else unpaid).
    paymentStatusManual: text('payment_status_manual'),
  },
  (t) => ({
...
const orderItems = pgTable(
  'order_items',
  {
    ...
    // Snapshot of the product's "X for Y" deal at order time (null = no deal).
    dealQty: integer('deal_qty'),
    dealPriceAgorot: integer('deal_price_agorot'),
    ...
    // Per-line discount applied at charge-review (resolved to agorot off the line).
    discountAgorot: integer('discount_agorot'),
    // VAT exemption snapshot at order time (for the invoice line).
    vatExempt: boolean('vat_exempt').notNull().default(true),
...
const payments = pgTable(
  'payments',
  {
    ...
    // Processor references only — NEVER raw card data. cardExpiry is the card's
    // MMYY (not sensitive like PAN/CVV) — Cardcom requires it alongside the token
    // when charging a saved card.
    providerCustomerRef: text('provider_customer_ref'),
