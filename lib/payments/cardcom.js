...
function publicConfig() {
  // Client-safe values for the OPEN FIELDS tokenizer (no secrets).
  return {
    provider: 'cardcom',
    terminalNumber: process.env.CARDCOM_TERMINAL || '',
    apiPublicKey: process.env.CARDCOM_API_PUBLIC || '',
  };
}
...
function buildDocument({ customer, items, description, externalId } = {}) {
  const c = customer || {};
  const products = toProducts(items, description);
  return {
    Name: c.fullName || c.name || 'לקוח',
    Email: c.email || '',
    IsSendByEmail: !!(c.email),
    Mobile: c.phone || '',
    ...
    Products: products,
    // DocumentTypeToCreate left default (0) → resolved from the admin panel
    // (typically חשבונית מס/קבלה). Override here if a specific type is needed.
  };
}

// Store the card on file. With OPEN FIELDS the browser already produced a reusable
// token, so the "save" is just adopting that token as our customerRef. (If the
// account is configured to return a SINGLE-USE token instead, this must first run
// a J5/J2 with create-token to obtain a reusable one — VERIFY once OPEN FIELDS +
// sandbox creds exist.)
async function saveCard({ singleUseToken, token, customer } = {}) {
  const t = token || singleUseToken;
  if (!t) return { ok: false, error: 'missing token' };
  void customer;
  return { ok: true, customerRef: String(t), cardLast4: '', brand: '' };
}

// Charge the saved token for the final amount and issue the invoice in one call.
async function charge({ customerRef, amountAgorot, description, items, customer, idempotencyKey, externalId } = {}) {
...
async function createDocument({ customer, items, description, externalId } = {}) {
...
module.exports = { publicConfig, saveCard, charge, refund, createDocument };
