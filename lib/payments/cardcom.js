...
function lineRepr(it, lineIls) {
  ...
  // accepts a decimal Quantity and uses TotalLineCost, so it stays exact. The
  // guard falls back to Quantity 1 if it wouldn't reconcile to the line total.
  if (it.isWeight && it.weightKg > 0 && u > 0 && round2(it.weightKg * u) === lineIls) {
  ...
  // Deals / estimate / fees → Quantity 1 with the exact total (keep a unit hint).
  const hint = it.isWeight ? ' (ק״ג)' : (u > 0 ? ' (יח׳)' : '');
  return { desc: it.name + hint + (qty !== 1 ? ' × ' + trimNum(qty) : ''), quantity: 1, unitCost: lineIls };
}
