...
// Description carries the item count so Quantity can stay 1 (see below).
function lineDesc(it) {
  const qty = Number(it.quantity);
  return it.name + (qty && qty !== 1 ? ' × ' + (+qty.toFixed(3)) : '');
}
// IMPORTANT: send Quantity = 1 with UnitCost = the exact line total. Sending the
// real quantity forces a rounded UnitCost, and Cardcom recomputes UnitCost ×
// Quantity — which drifts from the weighed line total and, accumulated over a
// large order, makes the document total ≠ the charge, so Cardcom rejects it
// ("Total items not equal to some form of payment"). Quantity 1 makes it exact.
...
function toInvoiceLines(items, description) {
  ...
  // Quantity 1 with Price = the exact line total — same reason as toProducts
  // (avoid UnitCost×Quantity rounding drift → "Total items not equal to payment").
  return list.map((it) => {
    const lineIls = lineIlsOf(it);
    return {
      Description: lineDesc(it),
