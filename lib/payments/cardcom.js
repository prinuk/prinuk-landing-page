...
async function createLowProfile({ operation, amountAgorot, description, returnValue, webhookUrl } = {}) {
...
async function saveCard({ singleUseToken, token, lowProfileId, customer } = {}) {
  ...
  return { ok: true, customerRef: String(r.token), cardLast4: r.cardLast4 || '', brand: '' };
}
