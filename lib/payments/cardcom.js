...
function toInvoiceLines(items, description) {
  ...
  return list.map((it) => {
    ...
    return {
      ...
      Quantity: qty,
      IsVatFree: it.vatExempt !== false,
      Price: round2(qty ? lineIls / qty : lineIls),
      TotalLineCost: round2(lineIls),
    };
  });
}
...
async function createTaxInvoice({ customer, items, description, externalId, invoiceType, payMethodId, cash, reference, amountAgorot, sendByEmail } = {}) {
  const c = customer || {};
  const body = {
    TerminalNumber: terminal(),
    ApiName: apiName(),
