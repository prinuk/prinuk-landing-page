...
// --- Document type mode (TEMPORARY, 2026-07) ------------------------------
// For now the business issues receipts (קבלה) only — NOT חשבונית מס קבלה. This
// will switch back in a few months. To revert: set CARDCOM_DOCUMENT_MODE=tax
// (or change the default below to 'tax'). One switch drives every document the
// adapter issues (card charge, POS charge, external-payment invoice, cash).
//   'receipt' → קבלה         (DocumentTypeToCreate 'Receipt' / InvoiceType 400)
//   'tax'     → חשבונית מס קבלה (terminal default / InvoiceType 1)
const DOCUMENT_MODE = (process.env.CARDCOM_DOCUMENT_MODE || 'receipt').toLowerCase();
const RECEIPT_DOC_TYPE = 'Receipt'; // Document.DocumentTypeToCreate (string enum) — קבלה
const RECEIPT_INVOICE_TYPE = 400;   // CreateTaxInvoice InvoiceType (numeric) — קבלה

// Charge / CreateDocument paths: the DocumentTypeToCreate to stamp when the caller
// doesn't force one. Receipt mode → 'Receipt' (קבלה); tax mode → null (unset, so
// Cardcom uses the terminal default, typically חשבונית מס קבלה).
function defaultChargeDocType() { return DOCUMENT_MODE === 'receipt' ? RECEIPT_DOC_TYPE : null; }

// CreateTaxInvoice: in receipt mode, remap a "חשבונית מס קבלה" request (type 1 /
// unset) to a plain קבלה (400). A pure חשבונית מס (305) or any other explicit
// type is left as the caller asked.
function resolveInvoiceType(requested) {
  const t = Number(requested) || 1;
  if (DOCUMENT_MODE === 'receipt' && t === 1) return RECEIPT_INVOICE_TYPE;
  return t;
}
...
function buildDocument({ customer, items, description, externalId, docType, paymentNote, sendByEmail } = {}) {
  ...
  // DocumentTypeToCreate: the caller's explicit type wins; otherwise the document
  // mode decides (receipt mode → 'Receipt' = קבלה; tax mode → unset, so Cardcom
  // uses the terminal default — typically חשבונית מס קבלה). See DOCUMENT_MODE.
  const resolvedDocType = docType || defaultChargeDocType();
  if (resolvedDocType) doc.DocumentTypeToCreate = resolvedDocType;
  return doc;
}
...
// NOTE: in receipt mode (DOCUMENT_MODE), resolveInvoiceType() remaps a type-1
// request to 400 (קבלה) — see the DOCUMENT_MODE block near the top.
async function createTaxInvoice({ customer, items, description, externalId, invoiceType, payMethodId, cash, dealNumber, reference, amountAgorot, sendByEmail, payments } = {}) {
  const c = customer || {};
  // Normalize payment tenders (card deal(s) + cash + external accounts). Back-compat:
  // the old single-mode params map to one tender.
  ...
  // Receipt mode: CreateTaxInvoice cannot emit a pure קבלה on our terminal — it
  // upgrades to חשבונית מס קבלה (verified). Issue the receipt via CreateDocument +
  // DocumentTypeToCreate 'Receipt' with the payment as a TOP-LEVEL Cash amount /
  // DealNumbers (verified: DocumentType comes back "Receipt"). A קבלה can't book to
  // a payment-method account (transfer/Bit) like CreateTaxInvoice's CustomLines, so
  // in this interim mode those are recorded as cash with the reference in Comments.
  if (DOCUMENT_MODE === 'receipt') {
    let cashSum = 0;
    const dealNumbers = [];
    const refs = [];
    tenders.forEach((t) => {
      if (t.kind === 'deal' && t.dealNumber) dealNumbers.push({ DealNumber: Number(t.dealNumber) });
      else if (t.kind === 'cash') cashSum += Number(t.amountAgorot || 0);
      else if (t.kind === 'custom') { cashSum += Number(t.amountAgorot || 0); if (t.reference) refs.push(String(t.reference)); }
    });
    const paymentNote = refs.length ? ('אסמכתא: ' + refs.join(', ')) : '';
    const body = {
      ApiName: apiName(),
      ApiPassword: apiPassword(),
      Document: buildDocument({ customer: c, items, description, externalId, docType: RECEIPT_DOC_TYPE, paymentNote, sendByEmail }),
    };
    if (dealNumbers.length) body.DealNumbers = dealNumbers;
    if (cashSum > 0) body.Cash = round2(cashSum / 100);
    const r = await post('/api/v11/Documents/CreateDocument', body);
    if (!r.ok) return r;
    const d = r.json || {};
    if (d.ResponseCode != null && Number(d.ResponseCode) !== 0) {
      return { ok: false, error: 'Cardcom ' + d.ResponseCode + ': ' + (d.Description || '') };
    }
    const di = d.DocumentInfo || {};
    return {
      ok: true,
      invoiceRef: String(d.DocumentNumber ?? di.DocumentNumber ?? ''),
      invoiceUrl: d.DocumentUrl ?? di.DocumentUrl ?? '',
      allocationNumber: d.TaxAuthorityAllocationNumber ?? di.TaxAuthorityAllocationNumber ?? '',
      json: r.json,
    };
  }

  // Tax mode: real חשבונית מס / חשבונית מס קבלה via CreateTaxInvoice (per-method
  // accounting via CustomLines for external payments).
  const body = {
    TerminalNumber: terminal(),
    ApiName: apiName(),
    ApiPassword: apiPassword(),
    InvoiceType: String(resolveInvoiceType(invoiceType)),
    InvoiceHead: {
