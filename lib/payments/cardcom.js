...
function lineRepr(it, lineIls) {
  ...
  // Weight-priced → "לפי משקל: <kg> ק״ג × ₪<price/kg>".
  if (it.isWeight && it.weightKg > 0 && u > 0) {
    return { desc: it.name + ' · לפי משקל: ' + trimNum(it.weightKg) + ' ק״ג × ₪' + u.toFixed(2), quantity: 1, unitCost: lineIls };
  }
  const qty = Number(it.quantity) || 1;
  // Unit-priced that reconciles → keep the real count × unit price, labelled "לפי יחידה".
  if (!it.isWeight && qty > 0 && u > 0 && round2(qty * u) === lineIls) {
    return { desc: it.name + ' · לפי יחידה', quantity: qty, unitCost: u };
  }
  // Everything else (deals/estimate/fees) → Quantity 1 with the exact total.
  const label = it.isWeight ? ' · לפי משקל' : (u > 0 ? ' · לפי יחידה' : '');
  return { desc: it.name + label + (qty !== 1 ? ' × ' + trimNum(qty) : ''), quantity: 1, unitCost: lineIls };
}
