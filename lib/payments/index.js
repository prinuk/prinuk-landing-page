...
// Default 'none' so production (no env set) keeps credit OFF. Set 'mock' on a
// preview to exercise the full flow with fake tokens; 'sumit' for the real one.
...
function getPaymentAdapter() {
  ...
  if (PROVIDER === 'cardcom') cached = require('./cardcom');
  else if (PROVIDER === 'sumit') cached = require('./sumit');
  else cached = require('./mock');
  return cached;
}

// Gate the customer-facing credit option on this so credit never shows before it
// can actually be processed. 'mock' counts (testable end-to-end on a preview).
function paymentsEnabled() {
  return PROVIDER === 'cardcom' || PROVIDER === 'sumit' || PROVIDER === 'mock';
}
