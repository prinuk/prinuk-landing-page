...
async function charge({ customerRef, amountAgorot, description, items, idempotencyKey } = {}) {
  ...
  const lineItems = (items && items.length)
    ? items.map((it) => ({
      Item: {
        Name: it.name,
        // VAT: produce is exempt; wine/delivery/etc. are taxable. Prices are
        // VAT-inclusive (see VATIncluded below). // VERIFY exact exemption field.
        ExemptVAT: it.vatExempt !== false,
      },
      Quantity: it.quantity || 1,
      UnitPrice: it.lineTotalAgorot != null ? (it.lineTotalAgorot / 100) / (it.quantity || 1) : undefined,
    }))
    : [{ Item: { Name: description || 'הזמנה', ExemptVAT: false }, Quantity: 1, UnitPrice: amount }];
  ...
  const r = await post('/billing/payments/charge/', { // VERIFY path
    ...
    Currency: 'ILS',
    VATIncluded: true, // web prices are gross; processor breaks out VAT. // VERIFY field
