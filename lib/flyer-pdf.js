...
function buildFlyerFooter(settings) {
  ...
  return [
    ...
    '<div class="footer-free">משלוח: 15 ש״ח, לשכונות נבחרות בירושלים בלבד. בהזמנה מעל 300 ש״ח המשלוח חינם.</div>',
    '</footer>',
  ].join('');
}
...
function buildFlyerCategoryHtml(category, breakBefore, twoCol) {
  ...
  const tbl = (arr) => '<table class="item-table"><tbody>' + arr.map(buildFlyerItemHtml).join('') + '</tbody></table>';
  let inner;
  if (twoCol && prods.length > 6) {
    const half = Math.ceil(prods.length / 2);
    inner = '<div class="two-col">' + tbl(prods.slice(0, half)) + tbl(prods.slice(half)) + '</div>';
  } else {
    inner = tbl(prods);
  }
  return [
    ...
    '<div class="category-items">', inner, '</div>',
    '</section>',
  ].join('');
}
...
function flyerCss() {
  return [
    ...
    '.flyer-header{display:table;width:100%;border-collapse:collapse;margin-bottom:8px;break-inside:avoid;page-break-inside:avoid;}',
    ...
    '.two-col{display:flex;gap:14px;align-items:flex-start;}.two-col .item-table{flex:1 1 50%;}',
    ...
    '.flyer-footer{margin-top:9px;border:1px solid #d9cdaa;border-radius:11px;background:#fff8e7;padding:7px 12px;text-align:center;font-weight:900;break-inside:avoid;page-break-inside:avoid;}',
    ...
    // per-category variant: each category on its own page (header + items +
    // footer flow naturally; page-break after each, no forced height).
...
function buildFlyerHtml(catalog, opts) {
  ...
  if (perCategory) {
    bodyInner = categories.map((cat) => (
      '<div class="cat-page">' + buildFlyerHeader(settings) + buildFlyerCategoryHtml(cat, false, true) + buildFlyerFooter(settings) + '</div>'
    )).join('');
  } else {
