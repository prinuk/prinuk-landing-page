...
function escapeHtml(value) {
  return String(value == null ? '' : value)
    // Serverless Chromium font lacks the gershayim/geresh glyphs → ASCII quotes.
    .replace(/״/g, '"')
    .replace(/׳/g, "'")
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
function buildOrdersFullHtml(orders, settings) {
  ...
  const blocks = orders.map((o) => {
    const rows = (o.items || []).map((it) => (
      '<tr><td>' + escapeHtml(it.name) + (it.note ? '<div class="inote">📝 ' + escapeHtml(it.note) + '</div>' : '') + '</td>' +
      '<td class="c">' + escapeHtml(it.quantity) + ' ' + escapeHtml(it.orderUnit || '') + '</td>' +
      '<td class="c">' + (it.lineTotal === '' || it.lineTotal == null ? '—' : money(it.lineTotal)) + '</td></tr>'
    )).join('');
    return [
      '<section class="order">',
      ...
      '<span>סה״כ: ', escapeHtml(o.totalText || money(o.grandTotal)), '</span>',
      '</div>',
      o.notes ? '<div class="onotes">📝 ' + escapeHtml(o.notes) + '</div>' : '',
      '<table><thead><tr><th>מוצר</th><th class="c">כמות</th><th class="c">סכום</th></tr></thead><tbody>', rows, '</tbody></table>',
      '</section>',
    ].join('');
  }).join('');
