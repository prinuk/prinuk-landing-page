...
function buildOrdersFullHtml(orders, settings) {
  const title = (settings && settings.saleName) ? 'הזמנות — ' + settings.saleName : 'הזמנות';
  const blocks = orders.map((o) => {
    const rows = (o.items || []).map((it) => {
      const wcell = it.isWeightPriced ? (it.actualWeightKg != null ? escapeHtml(it.actualWeightKg) + ' ק"ג' : '—') : '';
      const lineFinal = (it.actualLineTotal !== '' && it.actualLineTotal != null) ? it.actualLineTotal : it.lineTotal;
      return '<tr><td>' + escapeHtml(it.name) + (it.note ? '<div class="inote">📝 ' + escapeHtml(it.note) + '</div>' : '') + '</td>' +
        '<td class="c">' + escapeHtml(it.quantity) + ' ' + escapeHtml(it.orderUnit || '') + '</td>' +
        '<td class="c">' + wcell + '</td>' +
        '<td class="c">' + (lineFinal === '' || lineFinal == null ? '—' : money(lineFinal)) + '</td></tr>';
    }).join('');
    ...
    return [
      ...
      hasActual
        ? '<span class="final">סה״כ סופי (שקילה): ' + money(o.actualTotal) + '</span><span>משוער: ' + escapeHtml(o.totalText || money(o.grandTotal)) + '</span>'
        : '<span>סה״כ משוער: ' + escapeHtml(o.totalText || money(o.grandTotal)) + '</span>',
      '</div>',
      o.notes ? '<div class="onotes">📝 ' + escapeHtml(o.notes) + '</div>' : '',
      '<table><thead><tr><th>מוצר</th><th class="c">כמות</th><th class="c">משקל</th><th class="c">סכום</th></tr></thead><tbody>', rows, '</tbody></table>',
      '</section>',
    ].join('');
  }).join('');
...
async function createOrdersFullPdf(orders, settings) {
  return renderHtmlToPdf(buildOrdersFullHtml(orders, settings));
}
