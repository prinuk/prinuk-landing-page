...
function buildOrdersFullHtml(orders, settings, opts) {
  ...
  const blocks = orders.map((o) => {
    // Missing items are summarised in the banner below, so keep the table to
    // what was actually collected (avoids listing every חסר item twice).
    const rows = (o.items || []).filter((it) => it.pickStatus !== 'חסר').map((it) => {
    ...
    const missingNote = missingNames.length
      ...
      : '';
    // Cash orders not yet paid → the customer pays this amount on delivery/pickup.
    // Use the FINAL total (items − discount + delivery), not items-only.
    const isPaid = o.paymentStatusManual === 'paid' || o.paymentStatus === 'captured';
    const payable = hasActual
      ? (o.finalTotal !== '' && o.finalTotal != null ? o.finalTotal : o.actualTotal)
      : (Number(o.grandTotal) || 0);
    const podNote = (o.paymentMethod !== 'credit' && !isPaid && payable > 0)
      ? '<div class="pod">💵 לתשלום ' + (o.fulfillment === 'משלוח' ? 'במסירה' : 'באיסוף') + ': ' + money(payable) + '</div>'
      : '';
    // Delivery fee as its own line so the item totals add up to the grand total.
    const deliveryRow = (Number(o.deliveryFee) > 0)
      ? '<tr><td>משלוח</td><td class="c">—</td><td class="c">—</td><td class="c">' + money(o.deliveryFee) + '</td></tr>'
      : '';
    return [
      '<section class="order">',
      ...
      '<span>', o.paymentMethod === 'credit' ? '💳 אשראי' : '💵 מזומן', '</span>',
      hasActual
        // Final total INCLUDES delivery (and any discount): use finalTotal, not
        // actualTotal (items only), so it matches the item + משלוח rows.
        ? ('<span class="final">סה״כ סופי' + (opts.hideEstimate ? '' : ' (שקילה)') + ': ' + money(o.finalTotal !== '' && o.finalTotal != null ? o.finalTotal : o.actualTotal) + '</span>'
      ...
      missingNote,
      '<table><thead><tr><th>מוצר</th><th class="c">הוזמן</th><th class="c">בפועל</th><th class="c">סכום</th></tr></thead><tbody>', rows, deliveryRow, '</tbody></table>',
      '</section>',
    ].join('');
  }).join('');
  ...
  return [
    ...
    // When several orders are printed together, each starts on its own page.
    '.order + .order{break-before:page;page-break-before:always;}',
    ...
    '.pod{font-size:14px;font-weight:800;color:#8a5a00;background:#fff3d6;border:1px solid #eecf8a;border-radius:6px;padding:5px 9px;margin-bottom:6px;}',
    ...
    '</style></head><body>',
    opts.hideTitle ? '' : ('<h1>' + escapeHtml(title) + (opts.title ? '' : ' (' + orders.length + ')') + '</h1>'),
    blocks,
    '</body></html>',
  ].join('');
}
