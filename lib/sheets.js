...
// TEMP: 'אחר' placed before 'יינות ואלכוהול' by request. To revert, remove
// 'אחר' from this list (unlisted departments fall to the end automatically).
const CATEGORY_ORDER = ['ירקות', 'פירות', 'עלים', 'מיוחדים', 'אחר', 'יינות ואלכוהול'];
...
function defaultSettings() {
  return {
    ...
    // Optional top-of-page announcement banner (e.g. a new product). Empty text
    // hides it. announcementProductId (optional) makes the banner clickable and
    // scroll to that product's card.
    announcement: '',
    announcementProductId: '',
    logoUrl: '',
    notificationEmails: '',
    telegramBotToken: '',
    telegramChatId: '',
    ...
    // Weekly cutoff day for time-limited items (0=Sunday … 6=Saturday). They are
    // orderable until this weekday at orderCutoffEnforceTime, then closed for the
    // rest of the week and reopen when the new week starts (Sunday).
    orderCutoffDay: '3',
    // Weekly "order earlier next week" reminder banner. Shown from weeklyReminderDay
    // at weeklyReminderTime (Israel) through the end of the week, then resets.
    weeklyReminderEnabled: 'on',
    weeklyReminderDay: '3',
    weeklyReminderTime: '00:00',
    weeklyReminderText: 'כדאי להקדים! 🌿 הזמנות שמגיעות מוקדם בשבוע מקבלות את מבחר התוצרת המלא והטרי ביותר. בסוף השבוע ההזמנות מסופקות על בסיס המלאי הקיים בלבד.',
    // Delivery: order subtotal (₪) at/above which delivery is free, and the fee.
...
const PRODUCT_IMAGE_CANDIDATES = [
  ...
  { url: '/assets/produce/red-onion.jpg', names: ['בצל סגול'] },
...
function calculateLineTotal(mode, quantity, product) {
  ...
  if (mode === 'unit' && priceUnitType === 'kg') {
    // Estimate weight comes ONLY from the admin-defined per-unit weight — no
    // hardcoded name-based fallback.
    const estimatedWeightPerUnitKg = Number(product.estimatedUnitWeightKg || 0);
...
const FREE_DELIVERY_THRESHOLD = 300; // default; overridable via settings
const DELIVERY_FEE = 15; // default; overridable via settings

// Resolve the delivery threshold + fee from settings (falling back to defaults).
function resolveDelivery(settings) {
  const s = settings || {};
  const t = Number(s.freeDeliveryThreshold);
  const f = Number(s.deliveryFee);
  return {
    threshold: isFinite(t) && t > 0 ? t : FREE_DELIVERY_THRESHOLD,
    fee: isFinite(f) && f >= 0 ? f : DELIVERY_FEE,
  };
}

// options.allowOutOfStockIds: a Set of product ids that may stay in the order even
// though they're out of stock — used when EDITING an order, so a "special" item
// (out-of-stock / past-cutoff) already in the order is preserved (locked), not
// rejected. New orders never pass it, so they still can't add out-of-stock items.
function validateAndBuildOrder(payload, products, settings, options = {}) {
  const allowOutOfStockIds = options.allowOutOfStockIds || null;
  const productMap = {};
  products.forEach(p => { productMap[p.id] = p; });
  const deliveryRules = resolveDelivery(settings);
  ...
  items.forEach(item => {
    ...
    // Out-of-stock blocks NEW orders; on an edit, a special item already in the
    // order is allow-listed (locked) so it stays instead of being rejected.
    if (product.outOfStock && !(allowOutOfStockIds && allowOutOfStockIds.has(product.id))) {
      throw new Error('המוצר ' + product.name + ' אזל מהמלאי. יש לרענן ולנסות שוב.');
    }
  ...
  // ₪15 delivery under the free threshold, free at/above it, none for pickup.
  const deliveryFee = fulfillment === 'משלוח' && estimatedTotal < deliveryRules.threshold ? deliveryRules.fee : 0;
