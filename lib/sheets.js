...
async function readOrderForEdit(orderId, editToken) {
  ...
  const itemValues = itemsRes.data.values || [];
  const items = [];

  for (let i = 1; i < itemValues.length; i++) {
    if (String(itemValues[i][1] || '').trim() !== orderId) continue;
    items.push({
      name: String(itemValues[i][2] || '').trim(),
      mode: String(itemValues[i][4] || '').trim() === 'משקל' ? 'kg' : 'unit',
      quantity: Number(itemValues[i][5] || 0),
      note: String(itemValues[i][10] || '').trim(),
    });
  }
  ...
  return {
    ...
    order: {
      orderId,
      customer: { fullName: row[3] || '', phone: row[4] || '', email: row[14] || '' },
      fulfillment,
...
async function updateOrderInPlace(order, editToken) {
  ...
  await clearOrderItemRows(sheets, spreadsheetId, order.orderId);
  ...
  // Clear the old picking block; the caller re-appends the fresh one via
  // appendPickingOrder (whose has-order guard now passes since it's cleared).
  await clearPickingBlock(sheets, spreadsheetId, order.orderId);

  return { rowNumber, timestamp: now };
}

// Clear (not delete) the contiguous item rows for an order. Clearing avoids
// fragile row-index deletion; the blank gap is cosmetic and low-volume here.
async function clearOrderItemRows(sheets, spreadsheetId, orderId) {
  ...
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: ORDER_ITEMS_SHEET + '!A' + (first + 1) + ':' + columnLetter(ORDER_ITEM_HEADERS.length) + (last + 1),
  });
}

// Clear an order's block in the picking sheet (from its 'מספר הזמנה' header row
// up to the next order's header row). Robust against the free-form block layout.
async function clearPickingBlock(sheets, spreadsheetId, orderId) {
  ...
  await sheets.spreadsheets.values.clear({
    spreadsheetId,
    range: PICKING_SHEET + '!A' + (start + 1) + ':F' + end,
  });
}
