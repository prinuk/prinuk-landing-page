...
const PRODUCT_IMAGE_FALLBACK_URL = '/assets/produce-photo-bg.png';
const PRODUCT_IMAGE_CANDIDATES = [
  { url: '/assets/produce/potato-white.jpg', names: ['תפוא לבן', 'תפוח אדמה לבן', 'תפוא גורמה בייבי'] },
...
function getProductImageUrl(productName) {
  ...
  return best.score >= 300 ? best.url : PRODUCT_IMAGE_FALLBACK_URL;
}
...
async function writeOrder(order) {
  ...
  const itemRows = order.items.map(line => [
    ...
    [line.note, formatEstimatedWeightNote(line)].filter(Boolean).join('\n'),
  ]);
