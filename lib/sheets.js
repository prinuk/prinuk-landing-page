...
function validateAndBuildOrder(payload, products) {
  ...
  items.forEach(item => {
    ...
    // kg ordering is only allowed for items whose sale unit is ק״ג;
    // anything else falls back to whole units.
    const allowsKg = getUnitType(product.unit) === 'kg';
    const mode = item.mode === 'kg' && allowsKg ? 'kg' : 'unit';
