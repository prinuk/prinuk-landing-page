  ...
  return {
    ...
    order: {
      ...
      items: items.map(item => ({
        ...
        note: item.note,
        picked: item.pickStatus !== ITEM_PICK_MISSING,
...
// sheet) and resolve the order status server-side — any missing item makes it
// 'נאסף חלקית', otherwise 'נאסף'. Records who collected it and when.
async function updateOrderCollection(orderId, { member, items } = {}) {
  const sheets = await getSheetsClient();
  const spreadsheetId = getSpreadsheetId();
  const rowNumber = await findOrderRowNumber(sheets, spreadsheetId, orderId);
  ...
  const itemUpdates = existingItems.map(item => {
    ...
    return {
      ...
      values: [[picked ? ITEM_PICK_COLLECTED : ITEM_PICK_MISSING]],
    };
  });

  const status = anyMissing ? ORDER_STATUS_PARTIAL : ORDER_STATUS_COLLECTED;
  const now = new Date().toISOString();
