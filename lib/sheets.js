...
function buildColumnMap(headers) {
  const map = { name: 0, department: 1, unit: 2, priceUnit: null, price: 3, active: null, weight: null, image: null, details: null };
  ...
  headers.forEach((header, index) => {
    ...
    if (v === 'שם' || v === 'שם מוצר' || v === 'מוצר' || v === 'name' || v === 'product') {
      ...
      map.image = index;
    } else if (v.includes('פרטים') || v.includes('תיאור') || v === 'details' || v === 'description' || v === 'info') {
      map.details = index;
...
function parseProducts(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const columnDetails = columns.details === null ? '' : String(row[columns.details] || '').trim();
