...
// provides them. Products not in this week's tab are hidden (kept, never deleted).
//
// With { dryRun: true } it computes and returns the exact plan (what would
// update / be created / be hidden) WITHOUT writing — used by the dashboard to
// show a confirm so a renamed product isn't silently duplicated.
async function publishSale({ saleName, pricingSpreadsheetId, dryRun } = {}) {
  ...
  // Plan first — match each pricing row by normalized name. The same plan drives
  // both the preview and the commit, so the preview is exactly what's applied.
  const willUpdate = [];
  const willCreate = [];
  const willHide = [];
  for (const item of incoming) {
    if (byName.has(normalizeProductName(item.name))) willUpdate.push(item);
    else willCreate.push(item);
  }
  for (const p of existing) {
    if (!incomingNames.has(normalizeProductName(p.name)) && p.state !== 'hidden') willHide.push(p);
  }
  ...
  const summary = {
    ...
    added: willCreate.length,
    hidden: willHide.length,
    newNames: willCreate.map((x) => x.name),
    hiddenNames: willHide.map((p) => p.name),
  };
  ...
  await db.transaction(async (tx) => {
    for (const item of willUpdate) {
      ...
      const set = {
        priceAgorot: Math.round(item.price * 100),
      ...
      // NEVER touch weightPerUnitKg — the estimate weight is admin-managed only.
      // (The pricing sheet's "weight" column is not a per-unit weight and would
      // clobber the admin values.)
    ...
    for (const item of willCreate) {
      await tx.insert(productsT).values({
        ...
        priceAgorot: Math.round(item.price * 100),
        state: 'active',
        weightPerUnitKg: null, // set later in the admin (not from the pricing sheet)
        imageUrl: item.image || '',
      });
    }
    for (const p of willHide) {
